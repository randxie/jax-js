...
type CompiledNode = {
  ...
  inputNames: string[];
  outputNames: string[];
  run(vars: Map<string, Operand>): Operand[];
};

/**
 * Lower an ONNX graph into a jax-js-native executable graph.
 *
 * This resolves operator handlers and parses node attributes once up front, so
 * runtime execution only walks precompiled steps that call jax-js primitives.
 */
export class LoweredONNXModel {
  readonly model: ModelProto;
  readonly run: (
    inputs: Record<string, np.Array>,
    options?: ONNXRunOptions,
  ...
  constructor(modelBytes: Uint8Array<ArrayBuffer>) {
    ...
    const compiledNodes = compileGraphNodes(this.model.graph.node);
    this.run = loweredModelAsJaxFunction(
      this.model,
      compiledNodes,
      this.#initializers,
    );
...
function compileGraphNodes(nodes: NodeProto[]): CompiledNode[] {
  return nodes.map((node) => {
    const opType = node.opType;
    const handler = (onnxOps as Record<string, any>)[opType];
    if (!handler) throw new Error(`Unsupported ONNX operation: ${opType}`);
    const attrs = parseAttributes(node);
    const inputNames = [...node.input];
    const outputNames = [...node.output];
    return {
      opType,
      name: node.name,
      inputNames,
...
function loweredModelAsJaxFunction(
  model: ModelProto,
  compiledNodes: CompiledNode[],
  initializers: Map<string, Operand> = new Map(),
): typeof LoweredONNXModel.prototype.run {
  const graph = model.graph;
  if (!graph) {
    throw new Error("ONNX model has no graph");
  }
  ...
  return function (
    ...
    try {
      ...
      for (const node of compiledNodes) {
        const results = node.run(vars);

        if (verbose) {
          const inputs = node.inputNames.map((n) => {
            if (!n) return "_";
            const operand = vars.get(n)!;
            if (operand instanceof StaticArray)
              return `${n} StaticArray[${operand.shape}]`;
          ...
          const outputs = results.map((operand, i) => {
            if (operand instanceof StaticArray)
              return `${node.outputNames[i]} StaticArray[${operand.shape}]`;
            return `${node.outputNames[i]} ${operand.aval}`;
          });
          console.log(
            `${node.opType} ${node.name}\n` +
              `  ${inputs.join(", ")} -> ${outputs.join(", ")}`,
        ...
        for (const [i, name] of node.outputNames.entries()) {
          const result = results[i];
          if (debugStats.has(name))
            logDebugStats(name, operandToJax(operandRef(result)));
          validateTensorShape(name, result.shape, valueInfo);
      ...
      for (const name of returnedOutputs) vars.delete(name);
      return outputs;
    } finally {
      for (const operand of vars.values()) {
        if (!(operand instanceof StaticArray)) {
